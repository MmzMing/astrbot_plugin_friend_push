...
才通过 GitHub API 提交，封面与配置落在同一条提交里。
...
- **单提交原子**：封面与配置写进同一条提交（Git Data API）；任一步失败整笔放弃，不会出现"配置写了封面没传"或 `image` 指向不存在文件的中间态。
- **分支冲突就停**：读取后若有人推送了新提交，更新 ref 返回 409/422，插件放弃提交而不是覆盖对方改动。
